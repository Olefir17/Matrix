...
public class MatrixDialogs extends JDialog {
    ...
    private class MatrixTableModel extends AbstractTableModel {
        ...
        public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
            try {
                ...
                JOptionPane.showMessageDialog(MatrixDialogs.this, "Enter number");
            }
        }
    ...
    public MatrixDialogs() {
        ...
        up.add(new JLabel("Lines numer:"));
        up.add(height);
        up.add(new JLabel("Collums number:"));
        up.add(width);
        ...
        JButton exit = new JButton(new AbstractAction("Return") {
            @Override
            public void actionPerformed(ActionEvent e) {
                dispose();
            }
    ...
    private double[][] checkData() {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int j = 0; j < row.length; j++) {
                Double value = row[j];
                if (value == null) {
                    JOptionPane.showMessageDialog(this, "Enter numbers in lines " + (i + 1) + ", collums " + (j + 1));
                    return null;
                }
